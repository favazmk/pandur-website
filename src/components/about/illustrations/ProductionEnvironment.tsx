...
export default function ProductionEnvironment({ progress }: Props) {
  // Background Parallax (Moves slightly slower, 0 to -60%)
  const bgX = useTransform(progress, [0, 1], ["0%", "-60%"]);
  const bgY = useTransform(progress, [0, 1], ["0%", "-60%"]);

  // Midground Parallax (The actual factory line, 0 to -80% so the 5th panel ends up on screen)
  const midX = useTransform(progress, [0, 1], ["0%", "-80%"]);
  const midY = useTransform(progress, [0, 1], ["0%", "-80%"]);
  ...
  // MIX: 0 - 0.2
  const mixSpin = useTransform(progress, [0, 0.2, 0.3], [0, 360, 360]);
  
  // BAKE: 0.2 - 0.4
  const ovenGlow = useTransform(progress, [0.1, 0.3, 0.5], [0.3, 0.8, 0.3]);
  const ovenTrayX = useTransform(progress, [0.1, 0.5], ["-20%", "120%"]);
  
  // COOL: 0.4 - 0.6
  const fanSpin = useTransform(progress, [0.3, 0.7], [0, 720]);
  
  // QUALITY: 0.6 - 0.8
  const laserScale = useTransform(progress, [0.65, 0.75], [1, 1.5]);
  const laserOpacity = useTransform(progress, [0.65, 0.7, 0.75], [0, 1, 0]);
  
  // PACK: 0.8 - 1.0
  const packSeal = useTransform(progress, [0.8, 0.95], [1, 0.8]);
  const packSlide = useTransform(progress, [0.8, 1.0], ["0%", "100%"]);
    ...
    <div className="relative w-full h-full overflow-visible">
      ...
      */}
      ...
      >
        ...
        {/* ================= BACKGROUND LAYER ================= */}
        <div className="absolute top-0 left-0 w-full h-[500vh] md:w-[500vw] md:h-full
